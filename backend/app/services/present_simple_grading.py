...
from app.models import PresentSimpleAttempt, PresentSimpleAttemptQuestion
...
def grade_question(question: PresentSimpleAttemptQuestion) -> bool | None:
...
def grade_attempt(attempt: PresentSimpleAttempt) -> None:
    grades = [grade_question(question) for question in attempt.questions]
    _apply_grade_totals(attempt, grades)


def recompute_attempt_from_grades(attempt: PresentSimpleAttempt) -> None:
...
def _apply_grade_totals(
    attempt: PresentSimpleAttempt, grades: list[bool | None]
) -> None:
...
def topic_performance(attempt: PresentSimpleAttempt) -> list[dict]:
...
def automatic_observation(attempt: PresentSimpleAttempt) -> dict[str, list[str]]:
