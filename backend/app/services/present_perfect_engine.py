...
from app.models import PresentPerfectQuestion, PresentPerfectTopic
...
def select_balanced_questions(
    questions: list[PresentPerfectQuestion],
    *,
    count: int = 20,
) -> list[PresentPerfectQuestion]:
    """Selecciona `count` preguntas repartidas de forma equilibrada por tema."""
    if count < len(PresentPerfectTopic):
        raise ValueError("Se necesita al menos una pregunta por tema.")

    grouped: dict[str, list[PresentPerfectQuestion]] = defaultdict(list)
    for question in questions:
        if question.active:
            grouped[question.topic].append(question)

    topics = list(PresentPerfectTopic)
    ...
    selected: list[PresentPerfectQuestion] = []
