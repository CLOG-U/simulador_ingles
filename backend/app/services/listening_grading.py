...
from app.models import ListeningAttempt, ListeningAttemptQuestion
...
def grade_question(question: ListeningAttemptQuestion) -> bool | None:
...
def grade_attempt(attempt: ListeningAttempt) -> None:
    grades = [grade_question(question) for question in attempt.questions]
    _apply_grade_totals(attempt, grades)


def recompute_attempt_from_grades(attempt: ListeningAttempt) -> None:
...
def _apply_grade_totals(
    attempt: ListeningAttempt, grades: list[bool | None]
) -> None:
...
def topic_performance(attempt: ListeningAttempt) -> list[dict]:
...
def automatic_observation(attempt: ListeningAttempt) -> dict[str, list[str]]:
