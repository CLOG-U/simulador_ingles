...
from app.schemas.listening import ListeningConfigUpdate
from app.services import listening_service
from app.services.audit_service import log_audit

router = APIRouter(prefix="/admin", tags=["admin-listening"])
...
@router.patch("/listening/config")
async def update_config(
    body: ListeningConfigUpdate,
    admin: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
    config = await listening_service.get_config(db)
    ...
    await log_audit(
        ...
        action="LISTENING_CONFIG_UPDATED",
        target_type="exam",
        target_id=ExamType.LISTENING_PRACTICE.value,
        metadata=changes,
    )
    await db.commit()
    return await listening_service.get_visible_config(db)
...
@router.get("/listening/questions")
async def list_questions(
    ...
    query = select(ListeningQuestion)
    if topic:
        ...
        "items": [
            {
                ...
                "clip_key": question.clip_key,
                "clip_title": question.clip_title,
            }
...
async def update_question(
    ...
    await log_audit(
        db,
        actor_user_id=admin.id,
        action="LISTENING_QUESTION_UPDATED",
        target_type="listening_question",
...
async def list_attempts(
    ...
    query = (
        select(ListeningAttempt, User)
        .join(User, User.id == ListeningAttempt.user_id)
        .where(ListeningAttempt.mode == mode)
    )
    if student_id:
        query = query.where(ListeningAttempt.user_id == student_id)
    if status:
        query = query.where(ListeningAttempt.status == status)
    if date_from:
        query = query.where(ListeningAttempt.started_at >= date_from)
    if date_to:
        query = query.where(ListeningAttempt.started_at <= date_to)

    total = (
        await db.execute(select(func.count()).select_from(query.subquery()))
    ).scalar_one()
    result = await db.execute(
        query.order_by(ListeningAttempt.started_at.desc())
        ...
        "items": [
            {
                ...
                "exam_name": (
                    "Listening Practice"
                    if mode == listening_service.MODE_PRACTICE
                    else "Listening Exam"
                ),
...
@router.patch("/listening/attempts/{attempt_id}/questions/{question_id}/grade")
async def override_listening_grade(
    attempt_id: uuid.UUID,
    question_id: uuid.UUID,
    ...
    data = await listening_service.override_question_grade(
        db,
    ...
    await log_audit(
        db,
        actor_user_id=admin.id,
        action="LISTENING_GRADE_OVERRIDE",
        target_type="listening_attempt_question",
