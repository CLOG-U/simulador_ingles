...
router = APIRouter(prefix="/admin/verb-past", tags=["admin-verb-past"])


class VerbPastConfigUpdate(BaseModel):
...
async def update_config(
    body: VerbPastConfigUpdate,
    admin: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
    config = await verb_past_service.get_config(db)
    ...
    await log_audit(
        ...
        action="VERB_PAST_CONFIG_UPDATE",
        target_type="verb_past_config",
...
async def list_attempts(
    ...
    items = await verb_past_service.list_attempts_admin(db, mode=mode)
...
async def attempt_report(
    ...
    return await verb_past_service.serialize_admin_report(db, attempt_id)
...
async def override_question_grade(
    ...
    data = await verb_past_service.override_question_grade(
        db,
    ...
    await log_audit(
        db,
        actor_user_id=admin.id,
        action="VERB_PAST_GRADE_OVERRIDE",
        target_type="verb_past_attempt_question",
