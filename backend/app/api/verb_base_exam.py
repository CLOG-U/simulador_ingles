...
async def current_attempt(
    ...
    attempt = await verb_base_service.get_open_attempt(
        db, student.id, mode=verb_base_service.MODE_EXAM
    )
    if attempt is None:
        return None
    return verb_base_service.serialize_attempt(attempt, include_grades=False)
...
async def practice_status(
    ...
    return await verb_base_service.get_attempt_status(
        db,
        student.id,
        mode=verb_base_service.MODE_PRACTICE,
    )
...
async def start_practice(
    ...
    attempt = await verb_base_service.create_or_get_practice(db, student)
    return verb_base_service.serialize_attempt(attempt, include_grades=False)
...
async def restart_practice(
    ...
    attempt = await verb_base_service.restart_practice(db, student)
    return verb_base_service.serialize_attempt(attempt, include_grades=False)
...
async def abandon_practice(
    ...
    abandoned = await verb_base_service.abandon_open_practice(db, student.id)
    await db.commit()
...
async def get_practice_session(
    ...
    if attempt.mode != verb_base_service.MODE_PRACTICE:
    ...
    return verb_base_service.serialize_attempt(
        attempt,
...
async def check_practice_answer(
    ...
    if attempt.mode != verb_base_service.MODE_PRACTICE:
        raise AppError("NOT_FOUND", "Sesión de práctica no encontrada.", status_code=404)
    return await verb_base_service.check_practice_answer(
        db,
        attempt=attempt,
...
async def submit_practice(
    ...
    if attempt.mode != verb_base_service.MODE_PRACTICE:
        raise AppError("NOT_FOUND", "Sesión de práctica no encontrada.", status_code=404)
    submitted = await verb_base_service.submit_attempt(db, attempt)
    return verb_base_service.serialize_result(
        submitted,
        student=student,
        include_review=True,
    )
