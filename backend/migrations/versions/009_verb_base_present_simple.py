"""Añade el examen Verb Base Form y el examen Present Simple

Revision ID: 009_verb_base_present_simple
...
revision: str = "009_verb_base_present_simple"
...
def upgrade() -> None:
    ...
    op.create_index("ix_verb_base_attempts_user_id", "verb_base_attempts", ["user_id"])

    op.create_table(
        "verb_base_attempt_questions",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "attempt_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("verb_base_attempts.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.Column("snapshot_valid_base_answers", postgresql.JSONB(), nullable=False),
        ...
        sa.UniqueConstraint("attempt_id", "position", name="uq_verb_base_attempt_question_position"),
        sa.UniqueConstraint("attempt_id", "verb_id", name="uq_verb_base_attempt_question_verb"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("is_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("practice_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("question_count", sa.Integer(), nullable=False, server_default="14"),
        sa.Column("passing_percentage", sa.Integer(), nullable=False, server_default="70"),
        sa.Column("duration_minutes", sa.Integer(), nullable=True),
        sa.Column("review_policy", review_policy, nullable=False, server_default="FULL"),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column("exam_type", sa.String(32), nullable=False, server_default="present_simple_exam"),
    ...
    op.create_index("ix_present_simple_questions_topic", "present_simple_questions", ["topic"])
    ...
    op.create_table(
        ...
        sa.Column("expires_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("submitted_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("total_questions", sa.Integer(), nullable=False, server_default="14"),
        sa.Column("correct_answers", sa.Integer(), nullable=True),
        sa.Column("incorrect_answers", sa.Integer(), nullable=True),
        sa.Column("unanswered_answers", sa.Integer(), nullable=True),
        sa.Column("percentage", sa.Numeric(5, 2), nullable=True),
        ...
        sa.Column("passed", sa.Boolean(), nullable=True),
        sa.CheckConstraint("mode IN ('exam', 'practice')", name="ck_present_simple_attempt_mode"),
    )
    op.create_index("ix_present_simple_attempts_user_id", "present_simple_attempts", ["user_id"])

    op.create_table(
        "present_simple_attempt_questions",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "attempt_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("present_simple_attempts.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "source_question_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("present_simple_questions.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column("position", sa.Integer(), nullable=False),
        ...
        sa.Column("graded_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint(
            "attempt_id", "position", name="uq_present_attempt_question_position"
        ),
        sa.UniqueConstraint(
    ...
    op.execute(
        """
        INSERT INTO verb_base_config (id, is_enabled, question_count, passing_percentage, review_policy)
        SELECT gen_random_uuid(), false, 20, 70, 'FULL'
        WHERE NOT EXISTS (SELECT 1 FROM verb_base_config)
        """
    )
    op.execute(
        """
        INSERT INTO present_simple_config (
            id, is_enabled, practice_enabled, question_count, passing_percentage, review_policy
        )
        SELECT gen_random_uuid(), false, false, 14, 70, 'FULL'
        WHERE NOT EXISTS (SELECT 1 FROM present_simple_config)
        """
    )


def downgrade() -> None:
    op.drop_table("present_simple_attempt_questions")
    op.drop_table("present_simple_attempts")
    op.drop_table("present_simple_questions")
    op.drop_table("present_simple_config")
    op.drop_table("verb_base_attempt_questions")
    op.drop_table("verb_base_attempts")
    op.drop_table("verb_base_config")
    op.drop_constraint("ck_exam_access_type", "exam_access", type_="check")
    op.create_check_constraint(
        "ck_exam_access_type",
        "exam_access",
