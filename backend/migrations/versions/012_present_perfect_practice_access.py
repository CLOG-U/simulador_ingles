...
def upgrade() -> None:
    # Crear fila de acceso Present Perfect para estudiantes que aún no la tienen.
    op.execute(
        """
        ...
        WHERE u.role = 'STUDENT'
          AND NOT EXISTS (
            ...
            WHERE ea.user_id = u.id
              AND ea.exam_type = 'present_perfect_exam'
          )
        """
    )
    # Misma idea que Past Simple (005): dejar la práctica usable.
    op.execute(
        """
        UPDATE exam_access
        SET practice_enabled = TRUE
        WHERE exam_type = 'present_perfect_exam'
        """
    )
    op.execute(
        """
        UPDATE present_perfect_config
        SET practice_enabled = TRUE
