...
import type { PastSimpleQuestion, PastSimpleResult } from "../../lib/types";
...
export function PastSimplePracticeInstructionsPage() {
  const { data: status } = useQuery({
    queryKey: ["past-simple-practice-status"],
      ...
      <section className="card max-w-2xl space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          {status?.has_open_attempt && status.open_attempt_id ? (
            <>
              <Link
                to={`/student/practice/past_simple/sessions/${status.open_attempt_id}`}
...
export function PastSimplePracticeStartRedirect() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const fresh = searchParams.get("fresh") === "1";
  const startedRef = useRef(false);
  const { mutate, isPending, error } = useMutation({
    mutationFn: fresh ? pastSimpleApi.restartPractice : pastSimpleApi.startPractice,
    onSuccess: (session) => {
      navigate(`/student/practice/past_simple/sessions/${session.id}`, {
        replace: true,
      });
    },
  });
...
export function PastSimplePracticeSessionPage() {
  const { sessionId = "" } = useParams();
  const navigate = useNavigate();
  const [index, setIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [feedback, setFeedback] = useState<Record<string, PastSimpleQuestion>>({});
  const [submitting, setSubmitting] = useState(false);
  const initializedRef = useRef<string | null>(null);

  const { data: session, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["past-simple-practice", sessionId],
    queryFn: () => pastSimpleApi.getPractice(sessionId),
  ...
  const checkMutation = useMutation({
    mutationFn: () =>
      pastSimpleApi.checkPracticeAnswer(
        sessionId,
        question.id,
...
export function PastSimplePracticeResultPage() {
  const { sessionId = "" } = useParams();
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["past-simple-practice-result", sessionId],
    queryFn: () => pastSimpleApi.practiceResult(sessionId),
    enabled: Boolean(sessionId),
  });
  ...
  return (
    <AppShell title="Practice Result">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            to={`/student/practice/past_simple/results/${sessionId}/review`}
            className="btn-primary"
          >
            Review Answers
          </Link>
          <Link to="/student/practice/past_simple/start?fresh=1" className="btn-primary">
            Practice Again
          </Link>
          <Link to="/student/practice" className="inline-flex rounded-xl border px-4 py-2.5">
            Back to Practice
...
export function PastSimplePracticeReviewPage() {
  const { sessionId = "" } = useParams();
  ...
  return (
    <AppShell title="Practice Review">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            to={`/student/practice/past_simple/results/${sessionId}`}
            className="btn-primary"
          >
            Back to Result
          </Link>
