...
export function PresentPerfectInstructionsPage() {
  return (
    <AppShell title="Present Perfect Exam">
      <section className="card max-w-2xl space-y-4">
        <h2 className="text-xl font-semibold">Present Perfect Exam</h2>
        <p>
        ...
        <div className="flex flex-wrap gap-3">
          <Link to="/student/exams/present_perfect_exam/start" className="btn-primary">
            Start Exam
          </Link>
          <Link to="/student/exams" className="inline-flex rounded-xl border px-4 py-2.5">
            Back to Exams
...
export function PresentPerfectStartRedirect() {
  const navigate = useNavigate();
  const startedRef = useRef(false);
  const { mutate, isPending, error } = useMutation({
    mutationFn: presentPerfectApi.startAttempt,
    onSuccess: (attempt) => {
      navigate(`/student/exams/present_perfect_exam/attempts/${attempt.id}`, {
        replace: true,
      });
    },
  });
...
export function PresentPerfectExamPage() {
  ...
  const {
    ...
    queryKey: ["present-perfect-attempt", attemptId],
    queryFn: () => presentPerfectApi.getAttempt(attemptId),
  ...
  useEffect(() => {
    if (attempt?.status === "SUBMITTED") {
      navigate(`/student/exams/present_perfect_exam/results/${attempt.id}/review`, {
        replace: true,
      });
    }
    ...
    const updateTimer = () => {
      ...
      if (remaining === 0 && !autoSubmittingRef.current) {
        ...
        void Promise.allSettled(Object.values(saveQueuesRef.current))
          .then(() => presentPerfectApi.submit(attempt.id))
          .finally(() => {
            window.location.href = `/student/exams/present_perfect_exam/results/${attempt.id}/review`;
          });
      }
    };
    updateTimer();
  ...
  const queueSave = useCallback(
    (questionId: string, answer: string) => {
      ...
      const operation = previous
        .catch(() => undefined)
        .then(async () => {
          await presentPerfectApi.saveAnswer(
            attemptId,
            questionId,
            answer || null,
          );
          ...
          setSaveLabel("Saved");
          queryClient.setQueryData<PresentPerfectAttempt>(
            ["present-perfect-attempt", attemptId],
            (old) =>
              old
                ? {
                    ...old,
      ...
      {showSubmit && (
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="present-perfect-submit-title"
          >
            <h3 id="present-perfect-submit-title" className="text-lg font-semibold">
              Submit Present Perfect Exam?
            </h3>
            <p>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    await presentPerfectApi.submit(attemptId);
                    window.location.href = `/student/exams/present_perfect_exam/results/${attemptId}/review`;
                  } catch {
                    setShowSubmit(false);
                    setSaveLabel("Not saved");
                  }
...
function ResultSummary({ result }: { result: PresentPerfectResult }) {
  return (
    <ExamResultSummary
...
export function PresentPerfectResultPage() {
  const { attemptId = "" } = useParams();
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["present-perfect-result", attemptId],
    queryFn: () => presentPerfectApi.result(attemptId),
    enabled: Boolean(attemptId),
  });
  ...
  if (isError) {
    return (
      <RequestError
        title="Present Perfect Exam Result"
        error={error}
  ...
  if (isLoading || !data) {
    return (
      <ExamResultShell title="Present Perfect Exam Result">
        <p>Loading result…</p>
      </ExamResultShell>
    );
  }
  ...
  return (
    <ExamResultShell title="Present Perfect Exam Result">
      ...
      <ExamResultActions
        ...
        reviewPath={`/student/exams/present_perfect_exam/results/${attemptId}/review`}
      />
    </ExamResultShell>
  );
}

export function PresentPerfectReviewPage() {
  const { attemptId = "" } = useParams();
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["present-perfect-result", attemptId],
    queryFn: () => presentPerfectApi.result(attemptId),
  ...
  return (
    <ExamResultShell title="Review Answers">
      ...
      <ExamReviewActions
        resultPath={`/student/exams/present_perfect_exam/results/${attemptId}`}
      />
    </ExamResultShell>
  );
}
